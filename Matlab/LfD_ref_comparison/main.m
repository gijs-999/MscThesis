...
Fe  = @(t,y) environment(y,d,ke,be);
ode = @(t,y) EoM(y,Fc(t,y),Fe(t,y));
sol2 = ode15s(ode,[ref2.t(i1),T],y0);
...
% ante impact
Fc  = @(t,y) controller_pos_nofilter(y,t,ref1,kc,bc);
Fe  = @(t,y) environment(y,d,ke,be);
ode = @(t,y) EoM(y,Fc(t,y),Fe(t,y));
sol1 = ode15s(ode,[0,T],y0);
...
ylabel("velocity [m/s]")
grid on; box on;
legend("$\dot{u}$","$\dot{x}^d$","$\dot{x}^i$","$\dot{x}^{\tilde{f}}$")
